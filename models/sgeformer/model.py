...
class SGEformer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.enc_embedding1 = ETSEmbedding(configs.d_model, configs.d_model, dropout=configs.dropout)
        # Encoder
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.embed == True:
            for i in range(len(growths)):
                gre_embed = self.enc_embedding1(growths[i])
                gre.append(gre_embed)

            for i in range(len(seasons)):
                ses_embed = self.enc_embedding1(seasons[i])
                ses.append(ses_embed)
            growth, season, growth_dampings = self.decoder(gre, ses)
